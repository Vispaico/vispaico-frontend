...
export default function FeaturedWorkCard({ project }: { project: Project }) {
  ...
  return (
    <motion.div
      className="group relative overflow-hidden rounded-2xl shadow-lg bg-white border border-gray-200/80 aspect-4/3"
      ...
      <Link href={`/portfolio/${project.id}`} passHref legacyBehavior>
        ...
        >
          ...
          <Image
            ...
            className={`object-cover transition-opacity duration-500 ${isHovering && hasVideo ? 'opacity-0' : 'opacity-100'}`}
          />
