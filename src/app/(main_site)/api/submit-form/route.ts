// /app/(main_site)/api/submit-form/route.ts (Final Linting Fix)
...
export async function POST(req: NextRequest) {
  try {
    ...
    switch (body.formType) {
      case 'kickoff':
        // ... (kickoff logic is correct and unchanged)
        ...
        await resend.emails.send({
            ...
            attachments: [{ filename: `Contract-${projectNumber}.pdf`, content: contractPdf }, { filename: `Invoice-${projectNumber}.pdf`, content: invoicePdf }],
        });
        ...
        if (discountAmount > 0) {
            ...
            teamHtml += `<br><hr><h2>Discount Information</h2><p><strong>Discount Earned from Quiz:</strong> $${body.discount}</p>`;
        }
        await resend.emails.send({
      ...
      default:
        // --- THIS IS THE FIX ---
        // Using an inline comment to disable the rule for this specific, known edge case.
