...
const ROUTE_SEGMENT = 'freelance-web-design-truths';
...
export default async function FreelanceWebDesignTruthsStory({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const resolvedLocale = resolveLocale(locale);
  ...
  const articleMarkdown = String.raw`Here's what clients wish they knew before hiring.
