...
const ROUTE_SEGMENT = 'ai-tools-that-actually-help-clients';
...
export default async function AiToolsThatActuallyHelpClientsStory({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const resolvedLocale = resolveLocale(locale);
