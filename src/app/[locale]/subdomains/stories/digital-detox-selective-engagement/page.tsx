...
const ROUTE_SEGMENT = 'digital-detox-selective-engagement';
...
export default async function DigitalDetoxSelectiveEngagementStory({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const resolvedLocale = resolveLocale(locale);
