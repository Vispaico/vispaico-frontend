...
const ROUTE_SEGMENT = 'wordpress-to-react-migration-guide';
...
export default async function WordpressToReactMigrationGuideStory({ params }: { params: Promise<{ locale: string }> }) {
  const { locale } = await params;
  const resolvedLocale = resolveLocale(locale);
  ...
  const articleMarkdown = String.raw`But migration sounds scary. Expensive. Risky.
